...
enum SafeToSpendUseCase {
    ...
    static func calculate(
        ...
        let variableSpent =
            transactions
            .filter {
                $0.isExpense && period.start <= $0.timestamp
                    && $0.timestamp <= calendar.endOfDay(for: now)
                    && $0.recurringRule == nil
            }
        ...
        let recurringDue =
            recurringRules
            .filter {
                $0.active && $0.isExpense
                    && isRuleDue($0, in: period, calendar: calendar)
            }
            .reduce(Int64(0)) { $0 + $1.amountMinor }
